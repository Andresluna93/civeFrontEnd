...
import { citasAPI } from "../../services/services";
...
const Step3FechaHora = ({
  ...
  return (
    <div className="fade-in max-w-3xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-display font-bold text-gray-800 mb-2">
          Escoge el mejor Horario para tu cita
        </h2>
        <p className="text-gray-500">
          Horarios disponibles📅<strong>{asesor?.nombres}</strong>
        </p>
      </div>
...
const AgendarCita = () => {
  ...
  const { clienteId, asesorId, servicioId } = useParams();
  const HandleCrearCita = async () => {
    ...
    const response = await citasAPI.crear(datosCita);
